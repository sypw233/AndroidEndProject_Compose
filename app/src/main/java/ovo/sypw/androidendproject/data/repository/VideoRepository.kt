...
class VideoRepository(private val apiService: ApiService) {
     ...
     * 获取B站热门视频
     */
    fun getBilibiliPopular(page: Int = 1, pageSize: Int = 20): Flow<Result<BilibiliPopularResult>> = flow {
        try {
            val response = apiService.getBilibiliPopular(page, pageSize)
            if (response.code == 0 && response.data != null) {
                emit(Result.success(BilibiliPopularResult(
    ...
    fun getVideoList(page: Int = 1, pageSize: Int = 10): Flow<Result<VideoListResult>> = flow {
        try {
            ...
            emit(Result.success(VideoListResult(
                videos = videos,
